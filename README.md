...
ChrisT.Jie 的個人作品集網站——以 **Cyber Obsidian** 視覺語言呈現的全端工程與 UI/UX 設計作品。內容由 TypeScript 靜態資料驅動，多處文案支援 **GFM Markdown** 渲染，並支援靜態匯出、GitHub Pages 部署與 PWA 離線體驗。

## 功能概覽

- **首頁** — 個人簡介（Markdown）、統計卡片、關於我與導覽入口
- **作品集** (`/projects/`) — 分類篩選、精選排序、卡片 hover 影片預覽
- **專案詳情** (`/projects/[slug]/`) — Hero 輪播、Markdown 摘要、時程與角色、獎項、技術棧、外部連結、畫廊
- **經歷** (`/experience/`) — 工作經歷、學歷、證照、技能時間軸（說明與重點支援 Markdown）
...
| 語言 | TypeScript |
| 內容 | [react-markdown](https://github.com/remarkjs/react-markdown)、[remark-gfm](https://github.com/remarkjs/remark-gfm) |
...
```
app/                    # Next.js 頁面與路由
components/             # UI 元件（layout、projects、home、pwa、ui）
content/                # 靜態內容資料
  profile.ts            # 首頁個人檔與 SEO
  experience.ts         # 經歷、學歷、證照、技能
  projects/             # 作品集項目（index.ts 註冊與排序）
lib/                    # 型別、資產路徑、Hero 解析、Markdown、PWA 設定
public/                 # 靜態媒體（profile/、projects/{slug}/、icons/）
docs/                   # 內容模型與範本文件
scripts/                # PWA 圖示產生與驗證腳本
```
...
- [`docs/projects/fields.md`](docs/projects/fields.md) — 專案欄位與 UI 對照（含 `awards` 獎項區塊）
...
- [`docs/markdown.md`](docs/markdown.md) — Markdown 欄位撰寫與擴充指南
